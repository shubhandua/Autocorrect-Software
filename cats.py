...
def accuracy(typed, reference):
    ...
    if len(typed_words)==len(reference_words):
        range_iter = len(reference_words)
    if len(typed_words)<len(reference_words):
        range_iter=len(typed_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for words in word_list:
        difference = diff_function(typed_word, words, limit)
        if difference < lowest_difference:
            autocorrect = words
            lowest_difference = difference
    
    return autocorrect if lowest_difference <= limit else typed_word
    # END PROBLEM 5
...
def hidden_kittens(typed, reference, limit):
    ...
    if limit < 0:  
        ...
        to_remove = 1 + hidden_kittens(typed[1:], reference, limit-1)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    progress = correct_num / prompt_num
...
def fastest_words(match):
    ...
    [4, 1, 6]
    """
    player_indexes = range(len(match["times"]))  # contains an *index* for each player
    word_indexes = range(len(match["words"]))    # contains an *index* for each word
    # BEGIN PROBLEM 10
    fastest_words = [[] for _ in player_indexes]
    for word_index in word_indexes:
        fast_time = float('inf')
        fast_player = None
        for player_index in player_indexes:
            timeofplayer = match['times'][player_index][word_index]
            if timeofplayer < fastest_time:
                fast_time = timeofplayer
                fast_player = player_index
        fastest_words[fast_player].append(get_word(match, word_index)) 
